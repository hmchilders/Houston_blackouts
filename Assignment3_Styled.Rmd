...
```{r}
nrow(unique(homes_with_blackouts$geom))
```
...
#blackout_census <- st_join(homes_with_blackouts, census_income, by = geom) %>%
 # mutate(blackout = 'yes')
```
...
```{r}
ggplot()+
 geom_sf(data = combined$VNP46A1.A2021038.h08v05.001.2021039064328.tif == 'TRUE', aes(fill = median_income))
```
