...
tile5_Feb7 <- read_stars("data/VNP46A1.A2021038.h08v05.001.2021039064328.tif", package = 'stars') 
...
```{r}
cropped_data <- st_intersection(blackout,Houston)
cropped_data 
```
...
```{r}
dissolved_buffer <- buffer %>% 
  st_union()

```

```{r}
plot(dissolved_buffer)
```

-   find areas that experienced blackouts that are further than 200m from a highway

```{r}
areas_outside_buffer <- st_difference(cropped_data,dissolved_buffer)

```

```{r}
plot(areas_outside_buffer)
```
...
-   filter to homes within blackout areas\
    ...
    ```{r}
    homes_within_blackout <- st_within(areas_outside_buffer, buildings)
    ```

-   count number of impacted homes\

```{r include=TRUE}
nrow(homes_within_blackout)
```
...
-   spatially join census tract data with buildings determined to be impacted by blackouts
    ...
    Houston <- Houston %>% 
      st_transform('EPSG:3083')
    census_buildings <- st_intersection(cropped_data, census_income)
...
census_blackouts <- unique(census_buildings$GEOID)

BlackoutCentroids <- st_centroid(census_buildings)


```

```{r}
plot(census_buildings$geometry)
```
...
```{r}
final_map <- census_income[Houston,] 
final_map_test <- st_filter(census_income, Houston, .pred = st_within)

Centriods <- final_map %>% 
  st_centriods()

#tract_with_blackout <- st_join(census_buildings, final_map)

ggplot()+
 geom_sf(data = final_map, aes(fill = final_map$median_income))
 geom_point(data = BlackoutCentroids$geometry, color = 'red')
```

```{r}
tm_shape(final_map)+
  tm_fill(fill = "median_income")+
  tm_shape(BlackoutCentroids)+
  tm_dots(col = 'red')
```
